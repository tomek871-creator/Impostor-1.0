...
# --- DANE O GRACH ---
games = {}  # {code: {players: [{"id": ip, "name": nick}], host_id, started, word, impostor_id}}
...
def create():
    ...
    games[code]["players"].append({"id": player_id, "name": name})
    games[code]["host_id"] = player_id

    return redirect(f"/room/{code}?id={player_id}")
...
def room_redirect():
    ...
    # Sprawdź, czy nie ma już 8 graczy
    if len(game["players"]) >= 8:
        return render_template("base.html", page="error", message="Pokój jest pełny!")

    # Dodaj nowego gracza, jeśli nie ma go na liście
...
def start(code):
    ...
    return redirect(f"/room/{code}?id={impostor['id']}")
